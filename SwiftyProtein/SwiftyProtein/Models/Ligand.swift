...
class Ligand {
	...
	private func getBondsNode(atoms: [Int : [String]], bonds: [Int : [String]]) -> SCNNode {
		...
		for bond in bonds {
			//			print(bond)
			if bond.value.count > 1 {
				let firstAtomId = (Int(bond.value[0]) ?? 0) - 1
				let secondAtomId = (Int(bond.value[1]) ?? 0) - 1
				
				if firstAtomId >= 0 && secondAtomId >= 0 && atoms.count > firstAtomId && atoms.count > secondAtomId {
